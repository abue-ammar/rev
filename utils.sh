...
strip_libs() {
	...
	# Skip if arch is "all"
	if [ "$keep_arch" = "all" ]; then
		return 0
	fi
	
	# Determine which libs to remove
	local remove_libs=()
	case "$keep_arch" in
		"arm64-v8a")
			remove_libs=("armeabi-v7a" "x86" "x86_64")
			;;
		"armeabi-v7a")
			remove_libs=("arm64-v8a" "x86" "x86_64")
			;;
		"x86")
			remove_libs=("arm64-v8a" "armeabi-v7a" "x86_64")
			;;
		"x86_64")
			remove_libs=("arm64-v8a" "armeabi-v7a" "x86")
			;;
		*)
			return 0
			;;
	...
	# Delete unwanted lib folders
	if [ -d "$temp_dir/lib" ]; then
		for lib in "${remove_libs[@]}"; do
			if [ -d "$temp_dir/lib/$lib" ]; then
				rm -rf "$temp_dir/lib/$lib"
			fi
		done
	fi
	
	# Rezip APK
	local backup="${apk_path}.bak"
	mv "$apk_path" "$backup"
	
	if (cd "$temp_dir" && zip -qr "${CWD}/${apk_path}" .); then
		rm -f "$backup"
		rm -rf "$temp_dir"
		return 0
	else
...
build_rv() {
	...
	for build_mode in "${build_mode_arr[@]}"; do
		...
		if [ "$build_mode" = apk ]; then
			...
			# Strip libraries if riplib is enabled
			if [ "${args[riplib]}" = true ]; then
				strip_libs "$apk_output" "$arch"
			fi
